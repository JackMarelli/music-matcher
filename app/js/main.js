...
_app.detectPhase = () => {
    ...
    if(document.location.pathname.includes("end.html")){
        if(localStorage.playlist ){
            qs(".redirect-playlist").addEventListener("click", function() {window.location.replace(`https://open.spotify.com/playlist/${_app.playlistID}`)});
        }
...
_app.requestPlaylist = async (tracks)=> {
    const recommendedTracks = await _app.sdm.getPlaylist(_app.host.token, null, tracks);
...
_app.exportPlaylist = async () => {
    ...
    songs.forEach(e => { _app.songs.push(`spotify:track:${e.id}`) });
    _app.playlistID = await _app.sdm.exportPlaylist(_app.host.token, _app.host.id, _app.songs, "Playlist by Music Matcher");
