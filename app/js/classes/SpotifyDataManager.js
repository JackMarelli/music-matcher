...
export default class SpotifyDataManager {
  ...
  async loadSpotifyHostData() {
    ...
    if (this.#errorCode)
      document.getElementById("error").innerHTML =
        "Connection not esthablised correctly.";
  ...
  async #fetchEveryUserResponses(users, limit, token) {
    ...
    for await (let user of users) {
      ...
      const res = await this.#fetchWebApi(
        token,
        `v1/recommendations?limit=${limit}&seed_artists=${seeds}`,
        "GET"
      );
  ...
  async getArtists(token, genres = [], countries = null) {
    ...
    let result = await this.#fetchWebApi(
      token,
      `v1/search?q=%20genre:${seeds}&type=artist&limit=50&offset=3`,
      "GET"
    );
